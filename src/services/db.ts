...
export const db = {
  ...
  getProfileBySlug: (slug: string): Profile | undefined => {
    ...
    return profiles.find(p => p.slug === slug || slugifyTurkish(p.name) === slug || p.id === slug);
  },
  ...
  getFeaturedProfiles: (): Profile[] => {
    return getProfilesFromStorage().filter(p => p.packageType === 'DIAMOND' || p.packageType === 'PREMIUM');
  },
  ...
  getStats: () => {
    ...
    return {
      ...
      premiumCount: profiles.filter(p => p.packageType === 'PREMIUM').length,
      goldCount: profiles.filter(p => p.packageType === 'GOLD').length,
      silverCount: profiles.filter(p => p.packageType === 'SILVER').length
    };
  },
  ...
  getCitiesWithCounts: (): City[] => {
    ...
    return CITIES.map(c => ({
      ...
      count: profiles.filter(p => p.city.toLowerCase() === c.toLowerCase() || p.citySlug === slugifyTurkish(c)).length
    }));
  },
  getServices: (): string[] => SERVICES,
  getReviews: (profileId: string): Review[] => {
    return [
      { id: 'r1', profileId: profileId, userName: 'Ahmet K.', rating: 5, comment: 'Çok profesyonel ve nezih bir terapi oldu, teşekkür ederim.', date: 'Dün', isApproved: true },
      { id: 'r2', profileId: profileId, userName: 'Murat Y.', rating: 5, comment: 'Temiz ve kaliteli bir seanstı.', date: '3 gün önce', isApproved: true }
    ];
  },
