...
export function ShowcaseCard({ profile, index = 0 }: ShowcaseCardProps) {
  ...
  // Ekran görüntünüzdeki parlak neon renk paleti (Sarı, Kırmızı, Mavi, Yeşil, Pembe...)
  const neonPalette = [
    { border: '#facc15', glow: 'rgba(250, 204, 21, 0.55)' }, // Sarı / Altın
    { border: '#f43f5e', glow: 'rgba(244, 63, 94, 0.55)' },  // Kırmızı / Gül
    { border: '#38bdf8', glow: 'rgba(56, 189, 248, 0.55)' }, // Camgöbeği Mavi
    { border: '#34d399', glow: 'rgba(52, 211, 153, 0.55)' }, // Zümrüt Yeşili
    { border: '#ec4899', glow: 'rgba(236, 72, 153, 0.55)' }, // Neon Pembe
    { border: '#a855f7', glow: 'rgba(168, 85, 247, 0.55)' }, // Mor / Menekşe
    { border: '#fb923c', glow: 'rgba(251, 146, 60, 0.55)' }, // Turuncu
    { border: '#22d3ee', glow: 'rgba(34, 211, 238, 0.55)' }  // Turkuaz
  ];
  ...
  return (
    <div 
      className="relative rounded-2xl overflow-hidden bg-[#0a0d14] flex flex-col transition-all duration-300 hover:-translate-y-1 group"
      style={{
    ...
    >
      <Link to={`/profil/${profile.slug}`} className="block relative aspect-[3/3.8] overflow-hidden">
        ...
        <div className="absolute top-2 left-2 z-10 pointer-events-none">
          <span className="px-2 py-0.5 rounded-md bg-[#facc15] text-black font-black text-[10px] tracking-wide flex items-center gap-1 shadow-md">
            <span>💙</span> VIP
          </span>
        </div>
        ...
        <div className="absolute top-2 right-2 z-10 pointer-events-none">
          <span className="px-2 py-0.5 rounded-md bg-[#22d3ee] text-black font-black text-[10px] tracking-wide flex items-center gap-1 shadow-md">
            <span>✓</span> ONAYLI
          </span>
        </div>
        ...
        <div className="absolute bottom-2 right-2 z-10 pointer-events-none">
          <span className="px-2.5 py-0.5 rounded-lg bg-black/85 border border-[#facc15]/80 text-[#facc15] font-black text-xs shadow-lg">
            {price} ₺
          </span>
        </div>
      </Link>
      ...
      {/* Kart Alt Bilgileri */}
      <div className="p-3 flex flex-col justify-between flex-1 bg-[#0a0d14] space-y-2">
        {/* İsim ve Yaş */}
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          <a 
            ...
            className="py-1.5 px-2 bg-[#10b981] hover:bg-[#059669] text-black font-black text-xs rounded-xl flex items-center justify-center gap-1 shadow transition-all active:scale-95"
          >
            WhatsApp
          </a>
          <a 
            href={`tel:${cleanPhone}`}
            onClick={(e) => e.stopPropagation()}
            className="py-1.5 px-2 bg-[#172033] hover:bg-[#222f4c] text-white font-bold text-xs rounded-xl border border-slate-700/60 flex items-center justify-center gap-1 shadow transition-all active:scale-95"
          >
            Ara
          </a>
        </div>
